...
/** RT - sorted construction
 * @author tomharris44
 *
 */
public class RangeTreeOrgSorted {
	
	private RangeTreeNode root;
	private ArrayList<Coordinate> coordsy;

	public RangeTreeOrgSorted(ArrayList<Coordinate> coords) {
		...
		this.coordsy = (ArrayList<Coordinate>)coords.clone();
		
		// sort data set on y coordinates and store in new list
		Collections.sort(coordsy, Comparator.comparing(Coordinate::getY));
	 ...
	 * @param sorted: sorted coordinates of nodes in subtree rooted at this point
	 */
	public void generate_internal_rts(RangeTreeNode node, ArrayList<Coordinate> sorted) {
		...
		ArrayList<ArrayList<Coordinate>> lr = get_sorted_y(node.getX(), sorted);
		
		ArrayList<Coordinate> left = lr.get(0);
		ArrayList<Coordinate> right = lr.get(1);
		
		// recursively call function on left subtree
		if (node.getLeft() != null) {
			generate_internal_rts(node.getLeft(),left);
		}
		
		// recursively call function on right subtree
		if (node.getRight() != null) {
			generate_internal_rts(node.getRight(),right);
		}
		
		// generate WB-BST on Y as per sorted construction method
		if (sorted.size() > 1) {
			if (sorted.size() == 2) {
				RangeTreeNode new_root = new RangeTreeNode(sorted.get(1).getX(),sorted.get(1).getY(),null);
				RangeTreeNode new_child = new RangeTreeNode(sorted.get(0).getX(),sorted.get(0).getY(),new_root);
				...
				RangeTreeNode new_root = new RangeTreeNode(sorted.get(median).getX(),sorted.get(median).getY(),null);
				
				generate_wb_bst(new_root, sorted.subList(0, median), sorted.subList(median+1, sorted.size()));
	 ...
	 */
	public ArrayList<ArrayList<Coordinate>> get_sorted_y(int nodeX, ArrayList<Coordinate> sorted){
		...
		ArrayList<ArrayList<Coordinate>> lr = new ArrayList<ArrayList<Coordinate>>();
		lr.add(left);
		lr.add(right);
	...
	/** Query sorted RT tree using standard RT querying algorithm
